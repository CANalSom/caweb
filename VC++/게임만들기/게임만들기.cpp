// 게임만들기.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
/// 나와 상대, 음식
RECT g_me, g_you, g_food, g_p1, g_p2, g_p3;
/// 크기 값을 보관하는 변수
int g_my_size = 150;
/// 생명력을 보관하는 변수
int g_my_life = 5;
/// 게임 상태를 확인할 수 있는 변수 : FALSE(게임중), TRUE(게임종료)
BOOL g_gameover = FALSE; // 초기값 명시
/// 상대 이동 속도 결정 보관 변수
int g_interval = 1000;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    {
        ...
        // 왼쪽 경계
        if (g_me.left < min_val)
        {
            g_me.left = min_val;
            g_me.right = min_val + g_my_size;
        }
        // 오른쪽 경계
        if (g_me.right > max_x)
        {
            g_me.right = max_x;
            g_me.left = max_x - g_my_size;
        }
        // 위쪽 경계
        if (g_me.top < min_val)
        {
            g_me.top = min_val;
            g_me.bottom = min_val + g_my_size;
        }
        // 아래쪽 경계
        if (g_me.bottom > max_y)
        {
            g_me.bottom = max_y;
            g_me.top = max_y - g_my_size;
        }
